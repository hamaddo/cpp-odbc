...
public:
    ...
    deposit create(const deposit& deposit_) {
        ...
        query_builder <<
                      "insert into clients(name, term, annual)" << std::endl <<
                      "values" << std::endl << '(' <<
                      '\'' << deposit_.name << '\'' << ',' <<
                      deposit_.term << ',' <<
                      deposit_.annual <<
    ...
    deposit read(int id_) {
        ...
        retcode = SQLBindCol(statement, 1, SQL_C_LONG, &term, 0, nullptr);
        retcode = SQLBindCol(statement, 1, SQL_C_FLOAT, &annual, 0, nullptr);
